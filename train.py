...
def explain_gnn_with_shap(model, valid_loader, device, sample_count=5, nsamples=50):
    ...
    background = np.stack([d.x.cpu().numpy().flatten() for d in data_list[:sample_count]], axis=0)
    ...
    if isinstance(shap_out, list):
        ...
        M = anchor.x.numel()
        K = flat_arr.size // M
        reshaped = flat_arr.reshape(K, M)
        flat_shap = reshaped[pred]
    return flat_shap.reshape(anchor.x.shape), anchor
